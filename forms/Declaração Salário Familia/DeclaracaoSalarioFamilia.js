...
$(function () {
  ...
  if (WKNumState == 0 || WKNumState == 4 || WKNumState == 9) {
    ...
    $(document).on("click", "[data-save-dependente]", function () {
      ...
      var dt_nascimento_dependente_modal = $(
        "#dt_nascimento_dependente_modal"
      ).val();

      var grau_parentesco_dependente_modal = $(
        "#grau_parentesco_dependente_modal"
      ).val();
      var grau_instrucao_dependente_modal = $(
        "#grau_instrucao_dependente_modal"
      ).val();
      ...
      */
      ...
      $("#dt_nascimento_dependente___" + row).val(
        dt_nascimento_dependente_modal
      );

      $("#grau_parentesco_dependente___" + row).val(
        grau_parentesco_dependente_modal
      );
      $("#grau_instrucao_dependente___" + row).val(
        grau_instrucao_dependente_modal
      );
    ...
    $(document).on("click", ".btn-edit-dependente", function (e) {
      ...
      let dt_nascimento_dependente = $(
        "#dt_nascimento_dependente___" + row
      ).val();

      let grau_parentesco_dependente = $(
        "#grau_parentesco_dependente___" + row
      ).val();
      let grau_instrucao_dependente = $(
        "#grau_instrucao_dependente___" + row
      ).val();
...
function carregaTemplate() {
  ...
  $("input[name^='num_dependente___']").each(function (i, v) {
    ...
    const texto_Grau_Parentesco = $(
      "#grau_parentesco_dependente___" + row + " > option:selected"
    ).text();
    const texto_Grau_Instrucao = $(
      "#grau_instrucao_dependente___" + row + " > option:selected"
    ).text();
  ...
  fetch("templateAddDependente.html")
    ...
    .then((template) => {
      const rendered = Mustache.render(
        template,
        dadosDependente == null ? {} : dadosDependente
      );
      ...
      modalDependente = FLUIGC.modal(
        ...
        function (err, data) {
          if (err) {
            ...
            $("#cpf_dependente_modal").mask("000.000.000-00", {
              reverse: true,
            });
            var dt_nascimento_dependente_modal = FLUIGC.calendar(
              "#dt_nascimento_dependente_modal"
            );

            $("#grau_parentesco_dependente_modal").html(
              $("#grau_parentesco_dependente").html()
            );
            $("#grau_instrucao_dependente_modal").html(
              $("#grau_instrucao_dependente").html()
            );

            if (dadosDependente != null) {
              $("#grau_parentesco_dependente_modal").val(
                dadosDependente.grau_parentesco_dependente
              );
              $("#grau_instrucao_dependente_modal").val(
                dadosDependente.grau_instrucao_dependente
              );
            } else {
